...
def calculate_uptime_downtime_for_a_period(store_id, current_start_time_utc,current_end_time_utc):
    ...
    if valid_first_point != start_time_utc:
      ...
      #print('gok',prev_status_data,current_start_time_utc)
      if prev_status_data:
        ...
        #if just left timestamp for valid_first_point is 2hours away from valid_first_point, then we will take status as active,
        #since this is inside business timings and we can't rely on timestamp which is 2hrs far.
        if valid_first_point- prev_status_time>= timedelta(hours=2):
            prev_status= "active"
      else:
         prev_status= 'active'
    ...
    flag=0
    #to take care of last part i.e valid_last_point and it' previous time stamp, we are appending valid_last_point in status_data
    ...
    for timestamp, status in status_data:
      ...
      if flag==0:
        ...
        continue
      #other than 1st part
      #if diff btw 2 consecutive elements is more than 2hrs, then 1st status will be there for 1hr, and
      #remaining time is 'active' whatever the suitation is. because within business hours
      if timestamp - prev_time>= timedelta(hours=2):
        if prev_status== "active":
